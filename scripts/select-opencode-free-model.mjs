...
export function selectFreeModel(availableIds) {
  ...
  const id = FREE_MODEL_PRIORITY.find((candidate) => available.has(candidate))
  if (!id) {
    throw new Error('No approved OpenCode Zen free model is currently available.')
  }
  return `opencode/${id}`
}
...
export async function main() {
  ...
  console.log(`Selected free OpenCode model: ${model}`)
