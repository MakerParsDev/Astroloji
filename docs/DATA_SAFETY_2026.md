...
| FCM token and platform | Automatically generated; optional notifications | Yes | Deliver requested notifications and manage token lifecycle | Stored until token rotation, notification removal, account deletion, or provider retention rules | Firebase Cloud Messaging, Cloudflare | Declare device or other IDs as applicable; app functionality |
...
- Purchase token, JWT, FCM token, advertising ID, or service error body
