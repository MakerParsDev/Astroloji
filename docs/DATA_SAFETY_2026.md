...
| **Date of birth** for the stateless Personal Guidance Beta (`/chart/natal`, `/chart/guidance`) | Optional, user-selected immediately before calculation | **Yes — transmitted for personal guidance** | Real-time natal/transit calculation | **Ephemeral**: used in Worker memory for the request; not written to D1, R2, analytics, logs, or Android preferences by this feature | Cloudflare as service provider | Review under Personal info / Date of birth; mark optional and ephemeral if the current form permits; keep prominent in-app disclosure |
| Personal guidance calculation output | Generated from date and current target time | Yes, returned to requesting device | Provide three traceable reflection signals | Kept only in the current Personal Guidance screen state; replaced when the date changes, cleared by the user, and discarded when the screen destination is removed or the app process ends; never written to Android preferences | Cloudflare backend | App functionality; do not describe as stored profile data |
| **Saved birth date/time and birth city coordinates** (`PUT /users/me/birth-data`, separate from the stateless feature above) | Optional; user saves once during onboarding to enable personalized daily content and a real Ascendant/houses without re-entering birth data every session | Yes — transmitted once to save, never returned in plaintext afterward | Personalized daily content generation (Faz 0.1/0.2), Ascendant/Midheaven/house calculation | **Persisted, encrypted at rest**: AES-256-GCM in a dedicated `user_birth_data` D1 table (one row per user, one authenticated ciphertext covering birth timestamp + latitude + longitude + IANA time zone together, not per-field). `GET` returns only `time_certainty` and whether a row exists — never the decrypted date, time, or coordinates. Deleted immediately by account deletion (see below) or by `DELETE /users/me/birth-data` | Cloudflare D1, decrypted server-side only when generating content/charts | Review under Personal info / Date of birth **and** approximate or precise location (birth city coordinates are stored, encrypted); mark optional; disclose that it is now stored (encrypted), not ephemeral, and is deletable independently of the account |
...
| Structured daily feedback category | Optional: `resonated`, `partly`, or `not_today` only | Yes for analytics; last date/sign/category also remains on device | Content quality measurement and duplicate-prompt prevention | Last date/sign/category stays in DataStore until replaced or local app data deletion; analytics follows event retention. No free text | Firebase Analytics, Cloudflare | Declare App activity / Other user-generated content only if Console taxonomy requires; otherwise App interactions. Never claim a journal is collected |
...
| Google Play consumable purchase token, product ID, credit ledger entries | User buys a credit pack or spends credits on a feature | Yes | Billing verification, credit balance accounting, fraud prevention | Account-linked `credit_ledger` rows until deletion; Google Play keeps its own transaction records | Google Play Billing, Google Play Developer API, Cloudflare | Declare Purchase history; app functionality, fraud prevention, account management |
| Friend connection (paired user IDs), invite code and its redemption state | User generates/shares an invite code; another user redeems it | Yes, to the paired account only | Enable the friend graph and, once implemented, relationship-based content | Account-linked `friendships`/`invite_codes` rows until either party deletes their account or removes the connection; a friend only ever sees the other user's sign/language, never raw birth data or account identifiers beyond what this feature exposes | Cloudflare | Declare App activity / other user-generated content or identifiers as applicable; app functionality |
| AI-generated deep reading text; AI consultation chat messages | User requests a deep reading or sends a chat message, spending premium access or credits | Yes, to the LLM provider and Cloudflare | Generate personalized natal-chart interpretation and answer user questions in that context | Deep reading text is stored server-side per user until account deletion or a birth-profile change (which produces a new, separate stored reading); chat messages are not persisted server-side beyond the single request/response — conversation history is kept only on-device | Cloudflare, configured LLM provider (Cloudflare Workers AI by default) | Declare AI-generated content per Play's AI content policy and App activity as applicable; app functionality |
...
- In-app **Delete account and data** removes the server account/profile, notification token association, user-linked events/rewards/subscription/credit-ledger records handled by the app, friend connections and invite codes (`friendships`, `invite_codes`), any stored AI-generated deep reading (`deep-reading/{user_id}/` in object storage), the saved encrypted birth-data row (`user_birth_data`, see below), Firebase identity, and local app data as implemented.
- Google Play subscription cancellation remains a separate user action in Google Play.
- The stateless Personal Guidance Beta (`/chart/natal`, `/chart/guidance`) still does not retain the birth date it is called with — no database record from that specific feature to delete.
- The separate, optional **saved** birth date/time/city (`user_birth_data`) is deleted automatically as part of account deletion, and can also be deleted independently at any time via `DELETE /users/me/birth-data` without deleting the whole account.
