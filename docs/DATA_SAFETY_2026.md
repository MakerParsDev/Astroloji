...
- In-app **Delete account and data** removes the server account/profile, notification token association, user-linked events/rewards/subscription records handled by the app, the saved encrypted birth-data row (`user_birth_data`, see below), Firebase identity, and local app data as implemented.
