...
- `ENABLE_METADATA_PUBLISH` remains `false`; mutation authority is an exact workflow-run ID plus immutable correlation and an expiry of at most 300 seconds. Authorization variables are explicitly reset to `disabled/0/disabled` after each mutation flow.
...
Keep `ENABLE_METADATA_PUBLISH=false`. Generate a unique UUID `authorization_correlation`, freeze the exact merged `main` head SHA, and dispatch `.github/workflows/android-metadata.yml` in `publish` mode with the exact backup run ID/SHA/confirmation plus that immutable correlation. Before writing any repository authorization variable, query only the canonical repository's `android-metadata.yml` `workflow_dispatch` runs on `main` and require **exactly one** run whose run ID is unique, head SHA equals the frozen SHA, repository/ref are `MakerParsDev/Astroloji` / `main`, event is `workflow_dispatch`, and `display_title` encodes the expected `publish` mode plus the exact `authorization_correlation`; reject zero, multiple, mismatched, or ambiguous runs. Install unconditional Bash cleanup before authorization and reset `METADATA_PUBLISH_AUTH_RUN_ID=disabled`, `METADATA_PUBLISH_AUTH_EXPIRES_AT_EPOCH=0`, `METADATA_PUBLISH_AUTH_CORRELATION=disabled`, and `ENABLE_METADATA_PUBLISH=false`. Authorize only the exact workflow run ID and exact correlation, with expiry no more than 300 seconds (5 minutes) in the future. Wait for `authorize-mutation` success and `play-mutation` start, then explicitly close all four values. Dispatch failure, job-start failure, cancellation, confirmation timeout, and every handled exit retain cleanup. On Windows PowerShell, perform authorization writes inside `try` and reset all four variables in `finally` using `gh variable set METADATA_PUBLISH_AUTH_RUN_ID --body disabled`, `gh variable set METADATA_PUBLISH_AUTH_EXPIRES_AT_EPOCH --body 0`, `gh variable set METADATA_PUBLISH_AUTH_CORRELATION --body disabled`, and `gh variable set ENABLE_METADATA_PUBLISH --body false`. The workflow's final verifier independently reads back the exact closed values and fails otherwise.
...
Keep `ENABLE_METADATA_PUBLISH=false`. Generate a new unique UUID `authorization_correlation` for cleanup, freeze the current merged `main` head SHA, and dispatch `cleanup` with all frozen cleanup values plus that correlation. Before authorization, query canonical `workflow_dispatch` runs and require **exactly one** run matching the exact run ID, frozen head SHA, `MakerParsDev/Astroloji` / `main`, event `workflow_dispatch`, cleanup mode, and immutable correlation encoded in `display_title`; reject ambiguous or mismatched runs. Install the same unconditional Bash closure before setting any authorization variable. Set `METADATA_PUBLISH_AUTH_RUN_ID` to that exact workflow run ID, `METADATA_PUBLISH_AUTH_CORRELATION` to the exact correlation, and `METADATA_PUBLISH_AUTH_EXPIRES_AT_EPOCH` to no more than 300 seconds (5 minutes) in the future. After `authorize-mutation` success and `play-mutation` start, close `METADATA_PUBLISH_AUTH_RUN_ID=disabled`, `METADATA_PUBLISH_AUTH_EXPIRES_AT_EPOCH=0`, `METADATA_PUBLISH_AUTH_CORRELATION=disabled`, and `ENABLE_METADATA_PUBLISH=false`. Dispatch failure, job-start failure, cancellation, confirmation timeout, and success retain cleanup. On Windows PowerShell, wrap authorization in `try/finally` and execute the same four `gh variable set` cleanup commands in `finally`. The workflow's final independent read-back fails unless all four values are closed.
...
Freshly verify `main`, Play production `1102 completed / 1.0`, exactly two supported locales, monthly/weekly subscriptions, `ENABLE_METADATA_PUBLISH=false`, `METADATA_PUBLISH_AUTH_RUN_ID=disabled`, `METADATA_PUBLISH_AUTH_EXPIRES_AT_EPOCH=0`, `METADATA_PUBLISH_AUTH_CORRELATION=disabled`, and no secret leakage. Preserve the original checkout's existing `Astroloji/gradlew` mode-only change.
